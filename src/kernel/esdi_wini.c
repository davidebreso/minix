 ...
 */
  ...
  outb(DMA_EXEC, (int)  tp->dma >>  0);	/* address bits 0..7   */
  outb(DMA_EXEC, (int)  tp->dma >>  8);	/* address bits 8..15  */
  outb(DMA_EXEC, (int) (tp->dma >> 16));	/* address bits 16..19 */
  ...
  outb(DMA_EXEC, w_opcode == DEV_WRITE ? 0x44 : 0x4C);
